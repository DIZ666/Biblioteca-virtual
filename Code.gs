...
function inicializarBaseDatos() {
  ...
  if (!sheetPrestamos) {
    ...
    sheetPrestamos.getRange("A1:H1").setFontWeight("bold").setBackground("#071426").setFontColor("#ffffff");
        ...
        // --- PURGA Y LIMPIEZA COMPLETA DE BASE DE DATOS (v3.2.0 - GRADO COMPAÑERO 2° SUPREMO) ---
  try {
    ...
    // 5. Limpiar usuarios de prueba y FORZAR Grado 2 (Compañero) y Rol Administrador para Patricio Díaz
    var sheetUsuarios = ss.getSheetByName("Usuarios");
    if (sheetUsuarios && sheetUsuarios.getLastRow() > 1) {
      ...
      for (var u = dataU.length - 1; u >= 1; u--) {
        ...
        if (userEmail.indexOf("juan.silva") !== -1 || userEmail.indexOf("diego.perez") !== -1 || userEmail.indexOf("claudio") !== -1) {
          sheetUsuarios.deleteRow(u + 1);
        } else if (userEmail.indexOf("patricio.diaz") !== -1 || userEmail.indexOf("diaz.patricio") !== -1 || userEmail.indexOf("diazp") !== -1) {
  ...
  try {
    ...
    if (estado === "Activo") {
      ...
      return { 
        success: true, 
        estado: estado,
        message: "Solicitud de registro recibida con éxito. Su cuenta ha quedado en revisión por la Gran Secretaría / Administración para verificar su grado antes de activar su acceso."
      };
    }
  } catch(e) {
